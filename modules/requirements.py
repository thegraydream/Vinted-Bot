...
def createconfigfile(path, write=None, seeinlogs=True):
    try:
        with open(f'{path}', 'r') as f:None
    except:
        try:
            if seeinlogs == True:writeinlog(f'Creation of the configuration file "{path}".')
            with open(f'{path}', 'w') as f:
                if seeinlogs == True:writeinlog(f'The configuration file "{path}" has been created.')
                if write == None:None
                else:
                    f.write(f'{write}')
        except:
            if seeinlogs == True:writeinlog(f'An error occurred while creating the configuration file "{path}".')
...
# Default Config
def defaultconfig(path, category, name, names, seeinlogs=True):
    config = configparser.ConfigParser()
    config.read(path)
    try:config[category][name]
    except:
        try:
            if seeinlogs == True:writeinlog(f'Create the default configuration of "{name}" in the "{category}" category of the "{path}" file.')
            config.set(category, name, names)
            with open(path, 'w') as configfile:
                config.write(configfile)
                if seeinlogs == True:writeinlog(f'The default configuration for "{name}" has been set to "{names}" in the "{path}" file.')
        except:
            if seeinlogs == True:writeinlog(f'An error occurred while creating the default configuration of "{name}" in the category "{category}" of the file "{path}".')

def createcategory(path, category, seeinlogs=True):
    try:
        config = configparser.ConfigParser()
        ...
        if not config.has_section(category):
            if seeinlogs == True:writeinlog(f'Creation of the category "{category}" of the file "{path}".')
            config.add_section(category)
            if seeinlogs == True:writeinlog(f'The category "{category}" in the file "{path}" has been created.')
            with open(path, 'w', encoding='utf-8') as configfile:
                config.write(configfile)
    except:
        if seeinlogs == True:writeinlog(f'An error occurred while creating the category "{category}" in the file "{path}".')
...
def verifyfile(path, seeinlogs=True):
    try:
        if seeinlogs==True:writeinlog(f'check for the existence of the "{path}" file')
                        ...
                        if not product_url in f.read():
                            with open(f'temp/{product}.temp', 'a', encoding='utf-8') as f:
                                ...
                                if readconfig('settings/settings.ini', 'settings', 'discord') == "true":
                                    if not webhook == "":
                                        sendwebhook(webhook, readjson('settings/webhook.json', 'defaultwebhook', 'name'), product_title, f"**:moneybag: Price:** ```{product_price}```\n**:bookmark_tabs: Brand:** ```{product_brand}```\n**:balloon: Size:** ```{product_size}```\n**Url:** {product_url}",readjson('settings/webhook.json', 'defaultwebhook', 'profilepicture'), readjson('settings/webhook.json', 'defaultwebhook', 'color'), author=seller_name, authoricon=seller_picture, authorurl=seller_profile_link, imageurl=product_image_link)
...
def writeinlog(text):
    if readconfig('settings/settings.ini', 'settings', 'logs', False) == 'true':
        datenow = datetime.datetime.now()
